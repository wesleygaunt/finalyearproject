...
C = 1*pico;        %capacitor in 

V_in = 0.8:0.005:1.0;      %range of v_in required
T_PULSE = T_0*sigmoid((V_in-V_0)*compress);

dT_PULSE = diff(T_PULSE);
dV_in = diff(V_in);
numerical_approx_derivative = dT_PULSE./dV_in;

dT_PULSEdV_in = T_0*compress*sigmoid_derivative((V_in-V_0)*compress);
